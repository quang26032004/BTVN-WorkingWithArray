...
//hiển thị list product có category là 'Home'
const homeProducts = products.filter(function(product) {
    return product.category === "Home";
});
console.log(homeProducts);

//hiển thị list product có price > 50 và sắp xếp sản phẩm có giá từ cao xuống thấp
const over50 = products.filter(function(product) {
    return product.price > 50;
});

//hiển thị list order của Emma Davis (gợi ý: dùng 'Emma Davis' để tìm userId, dùng userId để lọc danh sách order)
const emma = users.find(function(user) {
    return user.name === "Emma Davis";
});
const emmaOrders = orders.filter(function(order) {
    return order.userId === emma.id;
});
console.log(emmaOrders);

//tính tổng tiền bill của Emma Davis (gợi ý: dùng reduce)
...
//hiển thị top 3 users chi tiêu mạnh nhất
const top3 = users.map(function(user) {
...
const emily = users.find(function(user) {
    return user.name === "Emily Johnson";
});
const emilyOrders = orders.filter(function(order) {
    return order.userId === emily.id;
});
const emilyTotal = emilyOrders.reduce(function(total, order) {
    const product = products.find(function(product) {
        return product.id === order.productId;
    });
    return total + product.price * order.quantity;
...
console.log(emmaTotal > emilyTotal ? "Emma Davis" : "Emily Johnson");

//kiểm tra xem có phải tất cả user đều có bill > 400
...
//hiển thị top 3 sản phẩm đc mua nhiều nhất
const top3Products = products.map(function(product) {
    const productOrders = orders.filter(function(order) {
        return order.productId === product.id;
    });
    const total = productOrders.reduce(function(total, order) {
        return total + order.quantity;
    }, 0);
    return { id: product.id, name: product.name, total };
}).sort(function(a, b) {
    return b.total - a.total;
}).slice(0, 3);
